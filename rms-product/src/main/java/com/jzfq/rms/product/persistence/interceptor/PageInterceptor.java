package com.jzfq.rms.product.persistence.interceptor;
...
public class PageInterceptor implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        if (delegate != null){
            ...
            if (page != null) {
                Object mappedStatement1 = ReflectUtil.getFieldValue(delegate, "mappedStatement");
                if (mappedStatement1 != null){
                    // 通过反射获取delegate父类BaseStatementHandler的mappedStatement属性
                    MappedStatement mappedStatement = (MappedStatement) mappedStatement1;
...
//        return "select count(*) from (" + sql + ") aliasForPage";
    ...
    private int getCount(String str, String key) {
        if (!StringUtil.isEmpty(str) && !StringUtil.isEmpty(key)) {
            int c = 0;
            int b = -1;
    ...
    private String getMysqlPageSql(PageParam<?> pageParam, String sql) {
        ...
        if (offset > 0) {
            stringBuilder.append(offset).append(",").append(pageSize);
        } else {
